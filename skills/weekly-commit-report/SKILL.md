...
3. Determine author filtering.
   ...
   - Do not hard-code author identities in the skill. By default, the script auto-detects the current machine's Git/GitHub identity from `git config` and the authenticated `gh` CLI user.
   - Pass explicit `--author` filters only when the user asks for a specific author or the auto-detected identities are incomplete.
   - Pass `--all-authors` only when the user asks for all commits regardless of author.
